...
```groovy
dependencies {
  compile("io.mybatis:mybatis-mapper:1.0.0-RC4")
  // 使用 Service 层封装时
  compile("io.mybatis:mybatis-service:1.0.0-RC4")
  // 使用 ActiveRecord 模式时
  compile("io.mybatis:mybatis-activerecord:1.0.0-RC4")
}
```
...
- `@Transient`: 排除字段，没有标记该字段的所有字段都会作为表字段处理（排除 static 和 transient 修饰的字段）
